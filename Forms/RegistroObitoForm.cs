...
{
    public partial class RegistroObitoForm : Form
    {
        ...
        public RegistroObitoForm(CartorioContext context)
        {
        ...
        private void InitializeUI()
        {
            ...
            // Add form fields
            AddFormField(formPanel, "Nome do Falecido:", out txtNome, row++);
            AddFormField(formPanel, "Data de Nascimento:", out dtpDataNascimento, row++);
            AddFormField(formPanel, "Data do Óbito:", out dtpDataObito, row++);
        ...
        private void SetupValidation()
        {
            ...
            txtNome.Validating += (s, e) => ValidationHelper.ValidateRequired(txtNome, "Nome do Falecido", errorProvider);
            dtpDataNascimento.Validating += (s, e) => ValidationHelper.ValidateDate(dtpDataNascimento, "Data de Nascimento", errorProvider);
            dtpDataObito.Validating += (s, e) => ValidationHelper.ValidateDate(dtpDataObito, "Data do Óbito", errorProvider);
            ...
            // Validação adicional para data de óbito
            dtpDataObito.Validating += (s, e) =>
            {
                if (dtpDataObito.Value.Date <= dtpDataNascimento.Value.Date)
                {
                    errorProvider.SetError(dtpDataObito, "Data do óbito deve ser posterior à data de nascimento.");
                }
                else
                {
                    errorProvider.SetError(dtpDataObito, "");
                }
            };
        }
        ...
        private void BtnSalvar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var registro = new RegistroObito
                {
                    DataRegistro = DateTime.UtcNow,
                    DataObito = DateTime.SpecifyKind(dtpDataObito.Value.Date, DateTimeKind.Utc),
                    Falecido = falecido
                };

                _context.RegistrosObito.Add(registro);
        ...
        private bool ValidateAll()
        {
            ...
            // Nome e Data de Nascimento
            isValid &= ValidationHelper.ValidateRequired(txtNome, "Nome do Falecido", errorProvider);
            isValid &= ValidationHelper.ValidateDate(dtpDataNascimento, "Data de Nascimento", errorProvider);
            isValid &= ValidationHelper.ValidateDate(dtpDataObito, "Data do Óbito", errorProvider);

            // Validação da data de óbito
            if (dtpDataObito.Value.Date <= dtpDataNascimento.Value.Date)
            {
                errorProvider.SetError(dtpDataObito, "Data do óbito deve ser posterior à data de nascimento.");
                isValid = false;
